...
from src.ir.model import Pipeline, Dataset, Operation, DataType, OpType
...
class TestIRValidation:
    ...
    def test_operation_must_reference_existing_datasets(self):
        ...
        """
        ds_a = Dataset(id="A", source="file", columns=[("col", DataType.INTEGER)])
    ...
    def test_pipeline_prevents_cycles(self):
        ...
        op1 = Operation(id="op1", type=OpType.COMPUTE, inputs=["A"], outputs=["B"])
        op2 = Operation(id="op2", type=OpType.COMPUTE, inputs=["B"], outputs=["A"])
